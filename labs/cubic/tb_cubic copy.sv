...
module tb_cubic;
    ...
    initial begin
        ...
        while (!$feof(file_handle)) begin
            ...
            // Read the float (This is usually where it fails!)
            scan_result += $fscanf(file_handle, "%f", y_float);
            
            // Read the final expected integer and float
            scan_result += $fscanf(file_handle, "%d", yint_expected);
            scan_result += $fscanf(file_handle, "%f", yfix_expected);

            if (scan_result == 7) begin
                // All good, run the test
                line_num++;
                // ... test logic ...
            end else begin
                $display("Error at Line %0d: Only captured %0d items correctly.", line_num+1, scan_result);
                void'($fgets(header_line, file_handle)); // Clear the broken line
            end
            // --- DEBUG END ---
            line_num++;
            num_tests++;
